...
func foldRight<U, V>(list: List<U>?, value: V, f: (U, V) -> V) -> V {
    if let listNode = list {
        return f(listNode.value, foldRight(list: listNode.next, value: value, f: f))
    } else {
        return value
    }
}
...
func foldLeft<U, V>(list: List<U>?, value: V, f: (U, V) -> V) -> V {
    if let listNode = list {
        return foldLeft(list: listNode.next, value: f(listNode.value, value), f: f)
    } else {
        return value
    }
}
...
func filterList<T>(list: List<T>, f: (T) -> Bool) -> List<T>? {
    if let nextNode = list.next {
        if f(list.value) {
            return List(value: list.value, next: filterList(list: nextNode, f: f))
        } else {
